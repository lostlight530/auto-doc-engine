...
> AST 驱动的科研文档编译、结构差异证据、跨文档诊断、SARIF 交换与可选 RO-Crate 1.3 研究对象打包

[English](README.md) · [架构](ARCHITECTURE_zh.md) · [科研契约](RESEARCH_CONTRACT.md) · [示例](examples/README_zh.md)

## 当前定位

`auto-doc-engine` 把 Markdown 当成**有结构、有来源、有边界的科研产物**，而不是等待字符串替换的一段文本
...
| `core/frontmatter.py` | **已实现** | title/description/authors/sources/license/doi/language/artifact_id 等有界科研元数据 |
...
未知字段仍按 warning 处理，方便未来扩展；类型和枚举错误保持显式错误

这不是完整学术元数据本体，只是给文档证据、跨仓 handoff 和研究对象打包提供稳定入口
...
SARIF 使用稳定 namespaced `ruleId` 与 `autoDocFinding/v1` partial fingerprint，把同一套诊断转换成标准结果容器；它不把 Markdown 体检包装成源码静态分析认证
...
这轮把 Markdown 输出从外部 `cp` 改成 Python 标准库复制，Windows / Linux / macOS 不再依赖同一个 shell 命令

`sync/targets.yaml` 中的 `pandoc_path` 现在真正参与运行时解析；Pandoc 缺失时明确报告，HTML 可以走 Mistune fallback，DOCX/PDF/EPUB 不伪造成功

## RO-Crate 1.3

RO-Crate 1.3 于 **2026-06-22** 发布，是本轮观察到的当前 long-term release

仓库现在不再只写“未来拟支持”，而是有真实 writer：
...
`core/incremental.py` 已统一使用 SHA-256 语义，并把重复 add/delete subtree 代码收敛为单一路径
...
历史记录改为原子替换写入，并限定每个文档最多保留最近 50 条摘要

这仍然只是 **change detector**，不负责自动 patch、冲突解决、所有权或多人协作合并

## AST 更新

`core/ast_engine.py` 本轮做了三类一致性修正：

- shallow signature 从 MD5 统一到 SHA-256
- Mistune 已启用的 `strikethrough` 现在真正有 AST 映射与 render 支持
- 增加 image / linebreak / ordered-list 等明确结构语义
- heading 文本抽取改成递归，格式化标题不再丢文字
- 不支持的 parse/render node 继续显式失败
...
- `template_prewarm.py`：调用者产物的有界 LRU cache，不再写“zero entropy”式能力声明
- `async_conduit.py`：有界 priority queue + concurrency scheduler，不是完整 streaming/backpressure 系统
- `memory_lattice.py`：本地 JSON node/link store + rounded numeric index，不是向量数据库或数学 lattice；load 时现在会真实重建索引
- `restart_protocol.py`：只有 handler 本身可重复时 replay 才可能确定；现在会真实比对 replay result hash，不再无条件返回验证成功
...
manifest、checksum、SARIF、RO-Crate 文件的存在都不能单独把结果提升到 R3
