...
The scanner does not mutate repository files, call GitHub, dereference remote
resources, run tests, or award scientific validity

`maintenance/cadence.yaml` is intentionally JSON-compatible YAML so this module
can parse it with the Python standard library only
"""
...
def build_report(
    *,
    ...
    return {
        ...
        "semantics": (
            "local deterministic maintenance evidence only; a clean report does not establish scientific truth, "
            "source credibility, standards certification, peer review, or independent reproduction"
        ),
    }
